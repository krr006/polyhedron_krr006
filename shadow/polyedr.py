...
class Facet:
    ...
    def area(self, k):
        ...
        for i in range(2, len(self.vertexes)):
            AB = self.vertexes[i-1] - self.vertexes[0]
            AC = self.vertexes[i] - self.vertexes[0]
            S = (AB.cross(AC).length())/2
            areas.append(S)
        area = sum(S for S in areas)
        return area / k**2
    ...
    #         AC = self.vertexes[i] - self.vertexes[0]
    #         BC = AC - AB
    #         p = 0.5 * (AB.length() + AC.length() + BC.length())
    #         S = sqrt(p*(p-AB.length())*(p-AC.length())*(p-BC.length()))
    #         areas.append(S)
    #     area = sum(S for S in areas)
    #     return area
    ...
    def is_center_outside_cube(self):
        c = self.center()
        expressions = [abs(c.x) > 0.5, abs(c.y) > 0.5, abs(c.z) > 0.5]
